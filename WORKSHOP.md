...
- THE TOOL IS THE LESSON. Each phase's checkpoint must happen in the real feature, never simulated by you in this coaching chat: Phase 2's "after" answer comes from a chat INSIDE their project (the project's Instructions doing the work — not you role-playing their voice here); Phase 3's artifact appears inside the connected tool; Phase 4's files are downloaded and opened, and the Skills page is visited; Phase 5 runs in Cowork with a real folder. If you can fake the result in this chat, it does not count — send them to the feature.
...
5. "Instructions" — click the pencil (edit) icon and paste (a) this scoped coach hook — "WORKSHOP HOOK: only when I type /phase3, /phase4, /phase5, /rules, or /help, fetch https://raw.githubusercontent.com/travcjohnson/claude-healthcare-workshop/main/WORKSHOP.md and coach me through that section. In all other chats, ignore this line and just be my practice assistant per the voice guide below." — followed by (b) their filled-in voice guide. Save.
...
## Step 4 · The after — inside the REAL project

Do not run this in the coaching chat, and do not answer it yourself — the project must do the work. Send them to their project tab: start a NEW chat INSIDE My Practice HQ (the project name shows at the top of the chat), paste the IDENTICAL test question there (give it again in ONE copy box so they don't scroll), and send it. That answer is generated by their project's Instructions — the actual feature they just built. Then have them bring the project-chat answer back here (paste it or summarize its opening lines) and compare it against the plain-chat "before": same facts, different practice. If the two sound alike, have them sharpen the "We always / We never" lines in the project's Instructions (pencil icon), save, and rerun in another new project chat.
...
2. Test material is the long FICTIONAL Linda dictation in the Card E block of the Prompt Pack. Remind them of the office rule that rides with that block: real notes get de-identified in a BAA-covered tool FIRST (for example Doximity GPT, free to verified clinicians); only de-identified text ever reaches Claude.
